...
export class SediciFileDownloadLinkComponent
  extends FileDownloadLinkComponent
  implements OnInit
{
  ...
  private getBitstreamPathObservable(): void {
    if (this.enableRequestACopy) {
      ...
      this.canDownloadWithToken$ = of(
        this.itemRequest &&
          this.itemRequest.acceptRequest &&
          !this.itemRequest.accessExpired
          ? this.itemRequest.allfiles !== false ||
              this.itemRequest.bitstreamId === this.bitstream.uuid
          : false
      );
  ...
  adaptFileSize(size: string): string {
    if (size.includes('KB')) {
      ...
      const formattedMbValue =
        roundedMbValue % 1 === 0
          ? roundedMbValue.toFixed(0)
          : roundedMbValue.toFixed(1); // Verificar si es un valor entero
      return `${formattedMbValue} MB`;
    } else if (size.includes('MB')) {
      const mbValue = parseFloat(size.replace('MB', '').trim()); // Extraer el valor numérico en MB
      if (mbValue >= 1000) {
        // Si el valor en MB es mayor o igual a 1000, convertir a GB
